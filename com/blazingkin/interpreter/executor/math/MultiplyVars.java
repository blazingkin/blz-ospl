...
public class MultiplyVars implements InstructionExecutorStringArray, LambdaFunction {
	...
	public void run(String[] args) {
		...
		if (Variable.isValInt(v1) && Variable.isValInt(v2)){
			...
			Variable.setValue(args[2], new Value(VariableTypes.Integer, i1.multiply(i2)));
		}
		if ((v1.type == VariableTypes.Integer || v1.type == VariableTypes.Double)
			...
			Value v = new Value(VariableTypes.Double, d1.multiply(d2));
	...
	public Value evaluate(String[] args) {
		...
		if (Variable.isValInt(v1) && Variable.isValInt(v2)){
			BigInteger i1 = Variable.getIntValue(v1);
			BigInteger i2 = Variable.getIntValue(v2);
			return new Value(VariableTypes.Integer, i1.multiply(i2));
		}
		if ((Variable.isValDouble(v1) || Variable.isValInt(v1))
				&& (Variable.isValDouble(v2) || Variable.isValInt(v2))){
			BigDecimal d1 = Variable.getDoubleVal(v1);
			BigDecimal d2 = Variable.getDoubleVal(v2);
			return new Value(VariableTypes.Double, d1.multiply(d2));
		}else{
			Interpreter.throwError("Invalid types for multiplication");
