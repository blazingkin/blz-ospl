...
public class ExponentVars implements InstructionExecutor {
	...
	public void run(String args[]){
		...
		if (Variable.isInteger(Variable.parseString(args[0])) && Variable.isInteger(Variable.parseString(args[1]))){
			v = new Value(VariableTypes.Integer, (int) Math.pow((Integer.parseInt(Variable.parseString(args[0]))),(Integer.parseInt(Variable.parseString(args[1])))));
		}else if((Variable.isDouble(Variable.parseString(args[0])) && Variable.isDouble(Variable.parseString(args[1])))){
			v = new Value(VariableTypes.Double, Math.pow((Double.parseDouble(Variable.parseString(args[0]))),(Double.parseDouble(Variable.parseString(args[1])))));		}else{
