	...
	public Value execute(Context con) throws BLZRuntimeException {
		...
		if (storeName == null){
			if (args[0].getOperator() == Operator.arrayLookup){
				...
				Value array = lookupNode.args[0].execute(con);
				if (array.type == VariableTypes.Array){
					...
					Variable.setValueOfArray(array, index, newVal, con);
					return newVal;					
				}else if (array.type == VariableTypes.String){
					Value newVal = args[1].execute(con);
					...
					String oldString = (String) array.value;
					...
					Value newVal = args[1].execute(con);
					Variable.setValueOfHash(array, key, newVal);
					return newVal;
				}
