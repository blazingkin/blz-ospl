...
public class WriteToResource implements InstructionExecutorCommaDelimited {
    ...
    public Value run(Value[] v) throws BLZRuntimeException{
        if (v.length != 2){
            throw new BLZRuntimeException("Passed incorrect number of arguments to write to resource");
        }
        if (v[0].type != VariableTypes.Resource) {
            throw new BLZRuntimeException("Cannot write to non-resource "+v[0]);
        }
        if (v[1].type != VariableTypes.String) {
            throw new BLZRuntimeException("Cannot write non-string "+v[1]);
        }
        ...
        try {
            ...
            throw new BLZRuntimeException("I/O Exception when writing to resource "+e.getMessage());
        }
        return v[0];
    }
